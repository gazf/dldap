...
async function makeCtx(
  ...
  // ベース DN と OU を事前に作成
  await store.set({ dn: "dc=example,dc=com", attrs: { objectclass: ["top", "domain"], dc: ["example"] } });
  await store.set({ dn: "ou=users,dc=example,dc=com", attrs: { objectclass: ["organizationalUnit"], ou: ["users"] } });
...
Deno.test("Bind: 匿名バインドは常に成功する", async () => {
  const { ctx, store, cleanup } = await makeCtx(false);
  try {
...
Deno.test("Bind: 管理者DNと正しいパスワードで成功", async () => {
  ...
  try {
    const result = await handleBind(
      { type: ProtocolOp.BindRequest, version: 3, dn: "cn=admin,dc=example,dc=com", password: "admin" },
      ctx,
    );
    assertEquals(result.response.result.resultCode, ResultCode.Success);
...
Deno.test("Bind: 管理者DNと誤ったパスワードは失敗", async () => {
  ...
  try {
    const result = await handleBind(
      { type: ProtocolOp.BindRequest, version: 3, dn: "cn=admin,dc=example,dc=com", password: "wrong" },
      ctx,
    );
...
Deno.test("Bind: 一般ユーザーのパスワード認証", async () => {
  ...
  try {
    ...
    const result = await handleBind(
      { type: ProtocolOp.BindRequest, version: 3, dn: "uid=alice,dc=example,dc=com", password: "alicepass" },
      guestCtx,
    );
    assertEquals(result.response.result.resultCode, ResultCode.Success);
  } finally {
...
Deno.test("Search SingleLevel: 直接の子のみ返す（孫は含まない）", async () => {
  const { ctx, store, cleanup } = await makeCtx();
  try {
    await store.set({ dn: "uid=john,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["john"] } });
    ...
    assertEquals(dns, ["ou=users,dc=example,dc=com"]); // john は孫なので含まない
  } finally {
    await cleanup();
  }
});
...
Deno.test("Search WholeSubtree: 全子孫を返す", async () => {
  ...
  try {
    await store.set({ dn: "uid=john,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["john"] } });
    await store.set({ dn: "uid=jane,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["jane"] } });
...
Deno.test("Search: sizeLimit が設定されている場合はそれ以上返さない", async () => {
  ...
  try {
    for (let i = 0; i < 5; i++) {
      await store.set({ dn: `uid=user${i},ou=users,dc=example,dc=com`, attrs: { objectclass: ["posixAccount"], uid: [`user${i}`] } });
    }
    ...
    await handleModify(
      {
        ...
        changes: [{ operation: ModifyOp.Replace, modification: { type: "cn", values: ["New Name"] } }],
      },
      ctx,
    );
    ...
    await handleModify(
      {
        ...
        changes: [{ operation: ModifyOp.Add, modification: { type: "mail", values: ["second@example.com"] } }],
      },
      ctx,
    );
    ...
    await handleModify(
      {
        ...
        changes: [{ operation: ModifyOp.Delete, modification: { type: "mail", values: ["a@example.com"] } }],
      },
      ctx,
    );
    ...
    await handleModify(
      {
        ...
        changes: [{ operation: ModifyOp.Replace, modification: { type: "userPassword", values: ["newpassword"] } }],
      },
      ctx,
    );
...
Deno.test("ModifyDN: エントリを別の OU に移動できる", async () => {
  const { ctx, store, cleanup } = await makeCtx();
  try {
    await store.set({ dn: "ou=archive,dc=example,dc=com", attrs: { objectclass: ["organizationalUnit"], ou: ["archive"] } });
    await store.set({ dn: "uid=move,ou=users,dc=example,dc=com", attrs: { uid: ["move"], cn: ["Move Me"] } });
...
Deno.test("Search AND フィルター: 両条件を満たすエントリのみ返す", async () => {
  const { ctx, store, cleanup } = await makeCtx();
  try {
    await store.set({ dn: "uid=alice,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["alice"], mail: ["alice@example.com"] } });
    await store.set({ dn: "uid=bob,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["bob"] } });

    const result = await handleSearch({
      type: ProtocolOp.SearchRequest,
      baseObject: "ou=users,dc=example,dc=com",
      scope: SearchScope.WholeSubtree,
      derefAliases: 0, sizeLimit: 0, timeLimit: 0, typesOnly: false,
      filter: {
        type: FilterTag.And,
        filters: [
...
Deno.test("Search OR フィルター: いずれかの条件を満たすエントリを返す", async () => {
  const { ctx, store, cleanup } = await makeCtx();
  try {
    await store.set({ dn: "uid=alice,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["alice"] } });
    await store.set({ dn: "uid=bob,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["bob"] } });
    await store.set({ dn: "uid=carol,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["carol"] } });
...
Deno.test("Search NOT フィルター: 条件を満たさないエントリのみ返す", async () => {
  ...
  try {
    await store.set({ dn: "uid=alice,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["alice"] } });
    await store.set({ dn: "uid=admin,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], uid: ["admin"] } });
...
Deno.test("Search Substrings フィルター: 前方一致・部分一致・後方一致", async () => {
  const { ctx, store, cleanup } = await makeCtx();
  try {
    await store.set({ dn: "uid=alice,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], cn: ["Alice Smith"] } });
    await store.set({ dn: "uid=bob,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], cn: ["Bob Jones"] } });
    await store.set({ dn: "uid=alan,ou=users,dc=example,dc=com", attrs: { objectclass: ["posixAccount"], cn: ["Alan White"] } });
